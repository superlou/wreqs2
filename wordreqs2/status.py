...
def find_next_id(reqs, config) -> pd.Series:
    reqs["prefix"] = reqs.doc_id.apply(lambda doc_id: config["docs"][doc_id].get("req_id_prefix", ""))
...
def run_status(reqs: pd.DataFrame, config: dict):
    ...
    next_req_id = find_next_id(reqs, config)
